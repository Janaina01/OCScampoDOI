...
{if $currentSchedConf->getSetting('metaDiscipline') || $currentSchedConf->getSetting('metaSubjectClass') || $currentSchedConf->getSetting('metaSubject') || $currentSchedConf->getSetting('metaDOI')|| $currentSchedConf->getSetting('metaCoverage') || $currentSchedConf->getSetting('metaType')}<p>{translate key="author.submit.submissionIndexingDescription"}</p>{/if}
...
<table width="100%" class="data">
	...
	{/if}
	{if $currentSchedConf->getSetting('metaDOI')}
	<tr valign="top">
		<td class="label">{fieldLabel name="doi" key="paper.doi"}</td>
		<td class="value">
			<input type="text" name="doi[{$formLocale|escape}]" id="doi" value="{$doi[$formLocale]|escape}" size="40" maxlength="255" class="textField" />
			{if $currentSchedConf->getLocalizedSetting('metaDOIExamples') != ''}
			<br />
			<span class="instruct">{$currentSchedConf->getLocalizedSetting('metaDOIExamples')|escape}</span>
			{/if}
		</td>
	</tr>
	<tr>
